  ...
  compareDeepIfVisited (additions, modifications, deletions, a, visited) {
    ...
    if (visited[id]) {
      ...
      if (!this.isEmpty(additions)) {
        for (const key in additions[additions.length - 1]) {
          if (!modifications[modifications.length - 1][key]) {
            if (Array.isArray(additions[additions.length - 1][key])) {
              modifications[modifications.length - 1][key] = []
            } else {
              modifications[modifications.length - 1][key] = {}
            }
          }
          if (!Array.isArray(additions[additions.length - 1][key])) {
            Object.assign(modifications[modifications.length - 1][key], additions[additions.length - 1][key])
          } else {
            modifications[modifications.length - 1][key].push(...additions[additions.length - 1][key])
          }
        }
        additions.length = 0
      }
